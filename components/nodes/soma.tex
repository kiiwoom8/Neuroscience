...
\node[default, draw, below=of third](different){Different mRNAs};
\draw[arrow, gray](rna) to (rna|-different) to node[below, switch ocg={alternativeSplicing alternativeSplicingg}](alternativeSplicing){
    \basic{alternativeSplicing}{Alternative Splicing}
}
(different);
\node at(alternativeSplicing){
    \basicr{alternativeSplicingg}{?}
};
\draw[dotted]($(rnap.east)+(.5,0)$) -- +(0,1) -| ($(rnap.west)+(-.5,0)$) -- ++(0,-1) -|
node[right, switch ocg={transcriptionFactor transcriptionFactorr}](transcriptionFactor){
...
% Gene
\node[default, above right=3 of nucleotides](gene2){};
...
\draw[arrow, dotted](nucleotides) -- (promotor2);